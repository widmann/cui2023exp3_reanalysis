...
describeBy(sgd_i ~ trial + cond, data = dat, digits = 1, mat = T)

dat_cond_trial <- dat %>%
  group_by(cond, trial) %>%
  summarise(sgd_i_sd = sd(sgd_i), sgd_i_mean = mean(sgd_i))
n <- nlevels(dat$subj)
ggplot(dat_cond_trial, aes(x = trial, y = sgd_i_mean, col = cond)) +
  geom_ribbon(aes(min = sgd_i_mean - sgd_i_sd / sqrt(n) * 1.96, max = sgd_i_mean + sgd_i_sd / sqrt(n) * 1.96, fill = cond), color = NA, alpha = 0.15) +
  geom_line() +
  geom_point() +
  scale_color_manual(values = okabe[c(6,8)]) +
  scale_fill_manual(values = okabe[c(6,8)]) +
  labs(color = "Condition", fill = "Condition") +
  labs(x = "Trial #", y = "Gaze disperson [a.u.]") +
  theme(legend.position = "bottom")
ggsave("sgd_i_time-on-task.pdf", device = "pdf", width = 12 / 2.54, height = 9 / 2.54)

# SNR
m1 <- sgd_i ~ snr_ctr * cond + (1 + cond | subj )
...
m1 <- sgd_i ~ snr_ctr * cond + trial_ctr + I(trial_ctr ^ 2) + (1 + cond | subj ) + (0 + trial_ctr + I(trial_ctr ^ 2) | subj)
fm1 <- lmer(m1, data = dat, control = lmerControl(calc.derivs = F), REML = F)
summary(fm1)
r.squaredGLMM(fm1)
tab_model(fm1, digits = 3)
...
ggplot(emm_df, aes(x = snr, y = yvar, col = cond)) +
  ...
  theme(legend.position = "bottom")
ggsave("sgd_i_snrxcond.pdf", device = "pdf", width = 12 / 2.54, height = 9 / 2.54)

# SNR x trial + SNR x cond
m2 <- sgd_i ~ snr_ctr * cond + snr_ctr * trial_ctr + (1 + cond | subj ) + (0 + trial_ctr | subj)
fm2 <- lmer(m2, data = dat, control = lmerControl(calc.derivs = F), REML = F)
summary(fm2)
r.squaredGLMM(fm2)

# SNR x trial x cond
m3 <- sgd_i ~ snr_ctr * trial_ctr * cond + (1 + cond | subj ) + (0 + trial_ctr | subj)
...
gg +
  ...
  theme(legend.position = "bottom")
ggsave("sgd_i_condxsnrxtrial-1.pdf", device = "pdf", width = 18 / 2.54, height = 12 / 2.54)
...
ggplot(emt_df, aes(x = trial, y = yvar, col = cond)) +
  ...
  scale_fill_manual(values = okabe[c(6,8)]) +
  labs(color = "Condition", fill = "Condition") +
  labs(x = "Trial #", y = "Change in gaze dispersion/+5 dB SNR [a.u.]") +
  theme(legend.position = "bottom")
ggsave("sgd_i_condxsnrxtrial-2.pdf", device = "pdf", width = 12 / 2.54, height = 9 / 2.54)

# Linked models
m1 <- sgd_i ~ trial_ctr * cond + I(trial_ctr ^ 2) * cond + (1 + cond | subj) + (0 + trial_ctr + I(trial_ctr ^ 2) | subj)
fm1 <- lmer(m1, data = dat, control = lmerControl(calc.derivs = F), REML = F)
summary(fm1)

dat$sgd_i_corr = dat$sgd_i - predict(fm1)

dat_cond_trial <- dat %>%
  group_by(cond, trial) %>%
  summarise(sgd_i_sd = sd(sgd_i_corr), sgd_i_mean = mean(sgd_i_corr))
...
m2 <- sgd_i_corr ~ snr_ctr * trial_ctr * cond + (1 | subj)
fm2 <- lmer(m2, data = dat, control = lmerControl(calc.derivs = F), REML = F)
summary(fm2)
